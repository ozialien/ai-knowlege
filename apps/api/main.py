...
from apps.api.schemas import (
    ...
    IngestRequest, PathIngestRequest, RAGRequest, RAGResponse, AgentRequest
)
...
from rag.retrieval import retrieve_contexts
from agents.simple_graph import build_graph

app = FastAPI(title="AI Platform V3", version="0.3.0")
graph = build_graph()
...
async def ingest_path(req: PathIngestRequest):
    ...
    return {"status": "ok", "chunks_indexed": count, "files_seen": len(set(p["path"] for p in payloads if p.get("path")))}
...
async def rag_answer(req: RAGRequest):
    ...
    return RAGResponse(answer=answer, contexts=context_texts)
